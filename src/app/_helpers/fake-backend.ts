    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function handleRoute() {
            switch (true) {
                ...
                case url.match('/posts') && method === 'POST':

                    return getAllPost(body?.id);
                case url.match('/comments') && method === 'GET':
                    return getAllComments();
                case url.match('/fb-accounts') && method === 'GET':
                    return getFBAccounts();
                default:
        ...
        function getAllPost(id: string) {
            const accessToken = localStorage.getItem(accessTokenKey);

            return from(new Promise(resolve => {
                fetch(`https://graph.facebook.com/v11.0/${id}/posts?access_token=${accessToken}`)
        ...
        function getFBAccounts() {
            const accessToken = localStorage.getItem(accessTokenKey);
            return from(new Promise(resolve => {
                fetch(`https://graph.facebook.com/v11.0/me/accounts?access_token=${accessToken}`)
                    .then(response => resolve(response.json()));
            })).pipe(concatMap((data: any) => {
                // if (data.error) return unauthorized(data.error.message);
